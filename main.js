...
const maps = [
    { url: 'https://www.leagueofgraphs.com/rankings/rank-distribution', element: document.getElementById('league'), map: league, label: 'League of Legends' },
...
function updatePercentiles() {
    ...
    maps.forEach(({ element, map, label, url }) => {
        element.innerHTML = `${label}: <br><i>${findPercentile(map, queryValue)}</i><br><a href="${url}">(source)</a>`;
    });
}

function findPercentile(map, queryValue) {
    let result = "(no data for value)"; 
